import { convertImageFolderPdf, downloadSoftUrl, Logger } from "@extrator/core";
import axios from "axios";
import cheerioModule from "cheerio";
const sleep = require("await-sleep");
// import { down } from "@extrator/core";

const logger = new Logger("download", "Extrator Manga");
...
export const extractManga = async (
  url: string,
  manga: string,
  parse: string,
  paseButton: string,
...
const extract = async (
  ...
  await downloadSoftUrl(
    await parsePages(url, manga, parse, paseButton, caps),
    pasta,
    manga,
    extensionFile
  );
